...
(function () {
        ...
        return {
            sendLog: function (e) {
                if (e = e.data && e.data.monitorUrl) {
                    var g = new Image, f = "log" + +new Date;
                    window[f] = g;
        ...
        var f = {
            text: function (b) {
                var c = '<span style="word-wrap:break-word;"><a href="{clickUrl:string}" target="{target:string}" style="font-size:{size:number}{unit:string};color:{defaultColor:string};font-weight:{defaultBold:string};font-style:{defaultItalic:string};text-decoration:{defaultUnderline:string};"{events}>{text:string}</a></span>',
                    a = /\{events\}/;
                if (1 === b.version)c = c.replace(a, ""); else if (2 === b.version)for (var c = c.replace(a, " onmouseover=\"this.style.color='{hoverColor:string}';this.style.fontWeight='{hoverBold:string}';this.style.fontStyle='{hoverItalic:string}';this.style.textDecoration='{hoverUnderline:string}';\" onmouseout=\"this.style.color='{defaultColor:string}';this.style.fontWeight='{defaultBold:string}';this.style.fontStyle='{defaultItalic:string}';this.style.textDecoration='{defaultUnderline:string}';\""), a = ["default",
                    "hover"], d = 0; d < a.length; d++) {
                    var i = a[d], g = i + "Color", f = i + "Bold", j = i + "Italic", i = i + "Underline";
                    b[g] = "#" + b[g];
                    b[f] = b[f] ? "bold" : "normal";
    ...
    window.BAIDU_DUP_define && window.BAIDU_DUP_define("clb/frame1418014201", ["api"], function (e) {
        function g() {
            var b = document.createElement("iframe"), a = o;
            b.src = "about:blank";
        ...
        function f() {
            var b = navigator.userAgent, a = b && b.match(/iphone.*micromessenger/i) ? m : o;
            f = function () {
                return a
            };
            return a
        ...
        function b() {
            return g() ? e.getConfig("domainPolicyFileUrl") || "/domain-policy.htm" :
                f() ? e.getConfig("blankPolicyFileUrl") || "/blank-policy.htm" : "about:blank"
        }

        e.defineOnce("BAIDU_DUP_CLB_renderFrame", function (c) {
            var a = document.getElementById("baidu_clb_slot_iframe_" + c), d = e.getSlots(c);
            if (d)if (g() && a.getAttribute("src", 2) !== b())a.src = b(); else if (d && !d.status.finish)try {
                ...
                f.write(i);
                f.close();
                f.body && (f.body.style.backgroundColor =
                    "transparent")
            } catch (l) {
        ...
        return {
            g: function (f) {
                !g && 64 !== f.slotType && (g = m, e.bind(window, "load", function () {
                    window.BAIDU_DUP_require(["clb/detect"], function () {
                    }, m)
                }))
            }
        }
    ...
    window.BAIDU_DUP_define && window.BAIDU_DUP_define("clb/fixed7o", ["api", "clb/monitor1418014201", "clb/material1418014201", "clb/frame1418014201", "clb/logService1418014201"], function (e, g, f, b, c) {
        return {
            render: function (a, d) {
                a = {id: a.id, data: a._html, k: a._fxp, i: a._stype, l: a._isMlt, width: a._w, height: a._h};
                c.g(a);
                g.sendLog(a);
                if (!a.data && !a.k)e.setStatus(a.id, "finish"); else {
                    ...
                    var i = document.getElementById(d),
                        p = f.a(a.data, a, m);
                    e.putInfo("#" + a.id, {html: p});
                    b.h(i, a)
                }
            }, validate: function (a) {
                var b = a._stype, a = a._isMlt;
        ...
        setTimeout(function () {
            ...
            for (c in a) {
                ...
                if (h.render) {
                    ...
                    if (i)e({type: "renderFail", id: c, error: "PermissionDenied", empty: !!d}); else if (h.finish) {
                        if (1 !== b.response.data._isMlt &&
                            (b = document.getElementById("baidu_clb_slot_iframe_" + c)))try {
                            var j = b.contentWindow.document.body;
                            (!j || !j.firstChild) && e({type: "renderEmpty", id: c})
                        } catch (k) {
                        }
                    } else e({type: "renderFail", id: c, error: "NotFilled", empty: !!d})
                }
            }
        }, 0)
    } catch (k) {
